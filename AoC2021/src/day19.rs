...
pub fn input_generator(input: &str) -> (Scanner, Vec<Point>) {
    ...
    create_omega_scanner(&scanners)
}

pub fn solve_part1(input: &(Scanner, Vec<Point>)) -> usize {
    input.0.points.len()
}

pub fn solve_part2(input: &(Scanner, Vec<Point>)) -> i64 {
    let mut largest_manhattan_distance = 0;
    for points in input.1.iter().combinations(2) {
        let distance = points[0].manhattan_distance(&points[1]);
        if distance > largest_manhattan_distance {
            largest_manhattan_distance = distance;
        }
