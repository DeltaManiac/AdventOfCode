...
type FoldType = (Option<usize>, Option<usize>);
type FoldList = Vec<FoldType>;

pub fn input_generator(input: &str) -> (Paper, FoldList) {
...
pub fn solve_part1(input: &(Paper, FoldList)) -> usize {
    let mut manual = input.0.clone();
    fold(&mut manual, input.1[0]);
    manual.len()
}

pub fn solve_part2(input: &(Paper, FoldList)) -> usize {
    let mut manual = input.0.clone();
